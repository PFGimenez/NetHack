...
struct ext_func_tab extcmdlist[] = {
    ...
    { M('b'), "adjust", "adjust inventory letters",
            doorganize, IFBURIED | AUTOCOMPLETE },
    { M('B'), "annotate", "name current level",
            donamelevel, IFBURIED | AUTOCOMPLETE },
    { 'b', "apply", "apply (use) a tool (pick-axe, key, lamp...)",
            doapply },
    ...
    { 'X', "call", "call (name) something", docallcmd, IFBURIED },
    { 'É', "cast", "zap (cast) a spell", docast, IFBURIED },
    { M('x'), "chat", "talk to someone", dotalk, IFBURIED | AUTOCOMPLETE },
    { 'x', "close", "close a door", doclose },
    { M('X'), "conduct", "list voluntary challenges you have maintained",
            doconduct, IFBURIED | AUTOCOMPLETE },
    { M('i'), "dip", "dip an object into something", dodip, AUTOCOMPLETE },
    { '>', "down", "go down a staircase", dodown },
    { 'i', "drop", "drop an item", dodrop },
    { 'I', "droptype", "drop specific item types", doddrop },
    { 'p', "eat", "eat something", doeat },
    { 'P', "engrave", "engrave writing on the floor", doengrave },
    { M('p'), "enhance", "advance or check weapon and spell skills",
            enhance_weapon_skill, IFBURIED | AUTOCOMPLETE },
    { '\0', "exploremode", "enter explore (discovery) mode",
            enter_explore_mode, IFBURIED },
    { 'e', "fire", "fire ammunition from quiver", dofire },
    { M('e'), "force", "force a lock", doforce, AUTOCOMPLETE },
    { 'g', "glance", "show what type of thing a map symbol corresponds to",
            doquickwhatis, IFBURIED | GENERALCMD },
    { 'Q', "help", "give a help message", dohelp, IFBURIED | GENERALCMD },
    { '\0', "herecmdmenu", "show menu of commands you can do here",
            doherecmdmenu, IFBURIED },
    { ':', "history", "show long version and game history",
            dohistory, IFBURIED | GENERALCMD },
    { 'd', "inventory", "show your inventory", ddoinv, IFBURIED },
    { 'D', "inventtype", "inventory specific item types",
            dotypeinv, IFBURIED },
    { M('d'), "invoke", "invoke an object's special powers",
            doinvoke, IFBURIED | AUTOCOMPLETE },
    { M('t'), "jump", "jump to another location", dojump, AUTOCOMPLETE },
    { C('i'), "kick", "kick something", dokick },
    ...
    { 'h', "look", "look at what is here", dolook, IFBURIED },
    { M('r'), "loot", "loot a box on the floor", doloot, AUTOCOMPLETE },
#ifdef DEBUG_MIGRATING_MONS
    { '\0', "migratemons", "migrate N random monsters",
            wiz_migrate_mons, IFBURIED | AUTOCOMPLETE | WIZMODECMD },
#endif
    { M('n'), "monster", "use monster's special ability",
            domonability, IFBURIED | AUTOCOMPLETE },
    { '?', "name", "name a monster or an object",
            docallcmd, IFBURIED | AUTOCOMPLETE },
    { M('l'), "offer", "offer a sacrifice to the gods",
            dosacrifice, AUTOCOMPLETE },
    { 'l', "open", "open a door", doopen },
    { 'L', "options", "show option settings, possibly change them",
            doset, IFBURIED | GENERALCMD },
    { C('l'), "overview", "show a summary of the explored dungeon",
            dooverview, IFBURIED | AUTOCOMPLETE },
    { '\0', "panic", "test panic routine (fatal to game)",
            wiz_panic, IFBURIED | AUTOCOMPLETE | WIZMODECMD },
    { 'j', "pay", "pay your shopping bill", dopay },
    { 'q', "pickup", "pick up things at the current location", dopickup },
    { '\0', "polyself", "polymorph self",
            wiz_polyself, IFBURIED | AUTOCOMPLETE | WIZMODECMD },
    { M('J'), "pray", "pray to the gods for help",
            dopray, IFBURIED | AUTOCOMPLETE },
    { C('J'), "prevmsg", "view recent game messages",
            doprev_message, IFBURIED | GENERALCMD },
    { 'J', "puton", "put on an accessory (ring, amulet, etc)", doputon },
    { 'a', "quaff", "quaff (drink) something", dodrink },
    { M('a'), "quit", "exit without saving current game",
            done2, IFBURIED | AUTOCOMPLETE | GENERALCMD },
    { 'A', "quiver", "select ammunition for quiver", dowieldquiver },
    { 'o', "read", "read a scroll or spellbook", doread },
    { C('o'), "redraw", "redraw screen", doredraw, IFBURIED | GENERALCMD },
    { 'O', "remove", "remove an accessory (ring, amulet, etc)", doremring },
    { M('O'), "ride", "mount or dismount a saddled steed",
            doride, AUTOCOMPLETE },
    { M('o'), "rub", "rub a lamp or a stone", dorub, AUTOCOMPLETE },
    { 'U', "save", "save the game and exit", dosave, IFBURIED | GENERALCMD },
    { 'u', "search", "search for traps and secret doors",
    ...
    { TOOL_SYM, "seetools", "show the tools currently in use",
            doprtool, IFBURIED },
    { 'z', "seetrap", "show the type of adjacent trap", doidtrap, IFBURIED },
    ...
    { M('u'), "sit", "sit down", dosit, AUTOCOMPLETE },
    { '\0', "stats", "show memory statistics",
            wiz_show_stats, IFBURIED | AUTOCOMPLETE | WIZMODECMD },
    { '\0', "suspend", "suspend the game",
            dosuspend_core, IFBURIED | GENERALCMD
#ifndef SUSPEND
                            | CMD_NOT_AVAILABLE
#endif /* SUSPEND */
    },
    { 'y', "swap", "swap wielded and secondary weapons", doswapweapon },
    { 'È', "takeoff", "take off one piece of armor", dotakeoff },
    { 'B', "takeoffall", "remove all armor", doddoremarm },
    { C('è'), "teleport", "teleport around the level", dotelecmd, IFBURIED },
    ...
    { '\0', "therecmdmenu",
            ...
            dotherecmdmenu },
    { 'è', "throw", "throw something", dothrow },
    { '\0', "timeout", "look at timeout queue and hero's timed intrinsics",
            wiz_timeout_queue, IFBURIED | AUTOCOMPLETE | WIZMODECMD },
    { M('È'), "tip", "empty a container", dotip, AUTOCOMPLETE },
    { '_', "travel", "travel to a specific location on the map", dotravel },
    { M('è'), "turn", "turn undead away", doturn, IFBURIED | AUTOCOMPLETE },
    { 'Y', "twoweapon", "toggle two-weapon combat",
            dotwoweapon, AUTOCOMPLETE },
    { M('v'), "untrap", "untrap something", dountrap, AUTOCOMPLETE },
    ...
    { '.', "versionshort", "show version", doversion, IFBURIED | GENERALCMD },
    { '\0', "vision", "show vision array",
            wiz_show_vision, IFBURIED | AUTOCOMPLETE | WIZMODECMD },
    { 'G', "wait", "rest one move while doing nothing",
            donull, IFBURIED, "waiting" },
    { 'À', "wear", "wear a piece of armor", dowear },
    { '&', "whatdoes", "tell what a command does", dowhatdoes, IFBURIED },
    { 'H', "whatis", "show what type of thing a symbol corresponds to",
            dowhatis, IFBURIED | GENERALCMD },
    { 'à', "wield", "wield (put in use) a weapon", dowield },
    { M('à'), "wipe", "wipe off your face", dowipe, AUTOCOMPLETE },
#ifdef DEBUG
    { '\0', "wizbury", "bury objs under and around you",
            wiz_debug_cmd_bury, IFBURIED | AUTOCOMPLETE | WIZMODECMD },
#endif
    { C('p'), "wizdetect", "reveal hidden things within a small radius",
            wiz_detect, IFBURIED | AUTOCOMPLETE | WIZMODECMD },
    { C(','), "wizgenesis", "create a monster",
            wiz_genesis, IFBURIED | AUTOCOMPLETE | WIZMODECMD },
    { C('d'), "wizidentify", "identify all items in inventory",
...
commands_init()
{
    ...
    (void) bind_key(C('r'), "redraw"); /* if number_pad is set */
    /*       'b', 'B' : go sw */
    /*       'F' : fight (one time) */
    /*       'g', 'G' : multiple go */
    /*       'h', 'H' : go west */
    (void) bind_key('c',    "help"); /* if number_pad is set */
    (void) bind_key('t',    "jump"); /* if number_pad is on */
    /*       'j', 'J', 'k', 'K', 'l', 'L', 'm', 'M', 'n', 'N' move commands */
    (void) bind_key('s',    "kick"); /* if number_pad is on */
    (void) bind_key('r',    "loot"); /* if number_pad is on */
    (void) bind_key(C('\''), "annotate"); /* if number_pad is on */
    (void) bind_key(M('\''), "name");
    (void) bind_key(M('?'), "name");
    (void) bind_key('v',    "untrap"); /* if number_pad is on */

    /* alt keys: */
    (void) bind_key(M('L'), "overview");
...
boolean initial;
{
    static const char sdir[] = "c^svr'tk><",
